...
def start_today_info(is_test=False):
    ...
    if dictum_channel == 1:
        dictum_msg = get_dictum_info()
    elif dictum_channel == 2:
        dictum_msg = get_ciba_info()
    elif dictum_channel == 3:
        dictum_msg = get_lovelive_info()
    else:
        dictum_msg = ''
...
def get_weather_info(dictum_msg, city_code, start_date, sweet_words):
    """
    ...
    if resp.status_code == 200 and is_json(resp) and resp.json().get('status') == 200:
